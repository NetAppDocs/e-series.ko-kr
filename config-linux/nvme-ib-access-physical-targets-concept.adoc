...
---
= E-Series-Linux(NVMe over InfiniBand)에서 물리적 NVMe 장치 타겟에 대한 NVMe 볼륨에 액세스
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
----
'NVMe list -Subsys' 명령을 사용할 때 물리적 NVMe 장치를 지정하면 해당 네임스페이스에 대한 경로에 대한 추가 정보가 제공됩니다.
...
NOTE: 이 방법은 RHEL 10에서는 더 이상 작동하지 않습니다. RHEL 9 이하 버전과 SLES 16 이하 버전에서 작동합니다.
